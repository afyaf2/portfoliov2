...
SizePlugin.prototype.stripHash = function stripHash (filename) {
    return this.reverseTemplate(filename, this.output.filename) || this.reverseTemplate(filename, this.output.chunkFilename) || filename;
};
SizePlugin.prototype.apply = function apply (compiler) {
    return new Promise((function ($return, $error) {
        ...
        if (compiler.hooks && compiler.hooks.afterEmit) {
            return $return(compiler.hooks.afterEmit.tapPromise(NAME, function (compilation) { return this$1.outputSizes(compilation.assets).catch(console.error); }));
        }
        return $return(compiler.plugin('after-emit', function (compilation, callback) {
            this$1.outputSizes(compilation.assets).catch(console.error).then(callback);
        }));
    }).bind(this));
};
SizePlugin.prototype.outputSizes = function outputSizes (assets) {
    return new Promise((function ($return, $error) {
        var sizesBefore, isMatched, isExcluded, assetNames, sizes, files, width, output;
                ...
                return Promise.all(assetNames.map(function (name) { return gzipSize(assets[name].source()); })).then((function ($await_2) {
                    ...
                    try {
                        ...
                        for (var i = 0, list = files; i < list.length; i += 1) {
                            ...
                            if (delta) {
                                var deltaText = (delta > 0 ? '+' : '') + prettyBytes(delta);
                                if (delta > 1024) {
